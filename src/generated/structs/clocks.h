...
#pragma once
#ifndef RP2040_STRUCTS_CLOCKS_H
...
struct clocks
{
    ...
    currently selected by the glitchless mux (one-hot). This slice does not
 ...
 currently selected by the glitchless mux (one-hot). The glitchless multiplexer
 does not switch instantaneously (to avoid glitches), so software should poll
 this register to wait for the switch to complete. This register contains one
 decoded bit for each of the clock sources enumerated in the CTRL SRC field. At
 most one of these bits will be set at any time, indicating that clock is
 currently present at the output of the glitchless mux. Whilst switching is in
 progress, this register may briefly show all-0s. */
 ...
 progress, this register may briefly show all-0s. */
  ...
  currently selected by the glitchless mux (one-hot). This slice does not have
  a glitchless mux (only the AUX_SRC field is present, not SRC) so this
  register is hardwired to 0x1. */
  ...
  currently selected by the glitchless mux (one-hot).  This slice does not have a
  glitchless mux (only the AUX_SRC field is present, not SRC) so this register
  is hardwired to 0x1. */
...
0 when not required Writing to this register initiates the frequency count */
    ...
    {
        return (CLK_GPOUT0_DIV >> 8u) & 0xffffffu;
    }
    ...
    {
        return (CLK_GPOUT1_DIV >> 8u) & 0xffffffu;
    }
    ...
    {
        return (CLK_GPOUT2_DIV >> 8u) & 0xffffffu;
    }
    ...
    {
        return (CLK_GPOUT3_DIV >> 8u) & 0xffffffu;
    }
    ...
    {
        return (CLK_SYS_DIV >> 8u) & 0xffffffu;
    }
    ...
    {
        return (CLK_RTC_DIV >> 8u) & 0xffffffu;
    }
