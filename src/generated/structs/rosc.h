...
{
    /* Constant attributes. */
    static constexpr struct_id_t id = 19;   /*!< rosc's identifier. */
    static constexpr std::size_t size = 36; /*!< rosc's size in bytes. */

    /* Fields. */
    uint32_t CTRL; /*!< (read-write) Ring Oscillator control */
    uint32_t
        FREQA;        /*!< (read-write) The FREQA & FREQB registers control the
       frequency by controlling the drive strength of each stage        The drive strength
       has 4 levels determined by the number of bits set        Increasing the number of bits
       set increases the drive strength and increases the oscillation frequency        0 bits
       set is the default drive strength        1 bit set doubles the drive strength        2 bits
       set triples drive strength        3 bits set quadruples drive strength */
 ...
 non-zero value. Can be used for short software pauses when setting up time
 sensitive hardware. */
     ...
     * because its frequency exceeds design specifications The clock output
     * will not glitch when changing the range up one step at a time The clock
     * output will glitch when changing the range down Note: the values here
     * are gray coded which is why HIGH comes before TOOHIGH
     */
     ...
     * before setting this field to DISABLE otherwise the chip will lock up The
     * 12-bit code is intended to give some protection against accidental
    ...
    {
        return ROSC_DIV_DIV(DIV & 0xfffu);
    }
