...
#pragma once
#ifndef RP2040_STRUCTS_XIP_CTRL_H
...
{
    /* Constant attributes. */
    static constexpr struct_id_t id = 31;   /*!< xip_ctrl's identifier. */
    ...
    uint32_t STREAM_FIFO;     /*!< (read-write) FIFO stream data
                ...
                exposing     the DMA to bus stalls caused by other XIP traffic. */
     ...
     * cache. When enabled, cacheable XIP accesses will query the cache, and
     * the flash will not be accessed if the tag matches and the valid bit is
     * set. If the cache is enabled, cache-as-SRAM accesses have no effect on
     * the cache data RAM, and will produce a bus error response.
     */
     ...
     * silently ignored. In either case, writes to the 0x0 alias will
     * deallocate on tag match, as usual.
     */
     ...
     * dissipation. Writing 1 to this bit forces CTRL_EN to 0, i.e. the cache
     * cannot be enabled when powered down. Cache-as-SRAM accesses will produce
     * a bus error response when the cache is powered down.
     */
     ...
     * Alternatively STAT can be polled until completion.
     */
    inline void set_FLUSH() volatile
    {
        FLUSH |= 1u;
    }
     ...
     * Alternatively STAT can be polled until completion.
     */
    inline void clear_FLUSH() volatile
    {
        FLUSH &= ~(1u);
    }
     ...
     * Alternatively STAT can be polled until completion.
     */
    inline void toggle_FLUSH() volatile
    {
        FLUSH ^= 1u;
    }
     ...
     * and also when requested via the FLUSH register.
     */
     ...
     * empty flag allow its level to be ascertained.
     */
     ...
     * transfer a linear data block from flash to the streaming FIFO.
     *                 Decrements automatically (1 at a time) as the stream
     *                 progresses, and halts on reaching 0.
     *                 Write 0 to halt an in-progress stream, and discard any
     * in-flight read, so that a new stream can immediately be started (after
