...
{
    /* Constant attributes. */
    static constexpr struct_id_t id = 14;      /*!< ppb's identifier. */
    static constexpr std::size_t size = 60836; /*!< ppb's size in bytes. */

    /* Fields. */
    std::array<const uint32_t, 14340> reserved_padding0 = {};
...
required every 100 clock pulses, set RELOAD to 99. */
    ...
    const uint32_t SYST_CALIB =
        {}; /*!< (read-only) Use the SysTick Calibration Value Register to
               ...
               multiply. */
    std::array<const uint32_t, 56> reserved_padding1 = {};
    uint32_t NVIC_ISER; /*!< (read-write) Use the Interrupt Set-Enable Register
to enable interrupts and determine which interrupts are currently enabled. If a
pending interrupt is enabled, the NVIC activates the interrupt based on its
...
interrupt, regardless of its priority. */
    ...
    std::array<const uint32_t, 95> reserved_padding5 = {};
...
highest priority, and 3 is the lowest. Note: Writing 1 to an NVIC_ICPR bit does
not affect the active state of the corresponding interrupt. These registers are
only word-accessible */
    ...
    uint32_t NVIC_IPR7; /*!< (read-write) Use the Interrupt Priority Registers
                           ...
                           available interrupts. 0 is the highest priority, and
                           3 is the lowest. */
    std::array<const uint32_t, 568> reserved_padding6 = {};
    ...
    uint32_t SHCSR; /*!< (read-write) Use the System Handler Control and State
                       ...
                       SVCall. */
    std::array<const uint32_t, 26> reserved_padding8 = {};
     ...
     * priority, and 3 is the lowest. Note: Writing 1 to an NVIC_ICPR bit does
     * not affect the active state of the corresponding interrupt. These
     ...
     * exception. This bit is WO. On a register read its value is Unknown.
     */
     ...
     * exception, it will activate as soon as it is registered. NMI set-pending
     * bit. Write: 0 = No effect. 1 = Changes NMI exception state to pending.
     *                 Read:
     *                 0 = NMI exception is not pending.
     *                 1 = NMI exception is pending.
     *                 Because NMI is the highest-priority exception, normally
     * the processor enters the NMI exception handler as soon as it detects a
     * write of 1 to this bit. Entering the handler then clears this bit to 0.
     ...
     * Thread mode. Setting this bit to 1 enables an interrupt driven
     ...
     * processor, disabled interrupts are excluded. 1 = Enabled events and all
     * interrupts, including disabled interrupts, can wakeup the processor.
     *                 When an event or interrupt becomes pending, the event
     * signal wakes up the processor from WFE. If the processor is not waiting
     * for an event, the event is registered and affects the next WFE. The
     * processor also wakes up on execution of an SEV instruction or an
     * external event.
     */
     ...
     */
    inline uint8_t get_SHPR2() volatile
    {
     ...
     * Priority of system handler 11, SVCall
     */
    inline void set_SHPR2(uint8_t value) volatile
    {
        uint32_t curr = SHPR2;
     ...
     */
    inline bool get_SHCSR() volatile
    {
        return SHCSR & (1u << 15u);
    }
     ...
     * to clear pending SVCall.
     */
    inline void set_SHCSR() volatile
    {
        SHCSR |= 1u << 15u;
    }
     ...
     * to clear pending SVCall.
     */
    inline void clear_SHCSR() volatile
    {
        SHCSR &= ~(1u << 15u);
    }
     ...
     * to clear pending SVCall.
     */
    inline void toggle_SHCSR() volatile
    {
        SHCSR ^= 1u << 15u;
    }
     ...
     * accesses use the default memory map. 0 = MPU disabled. 1 = MPU enabled.
     */
     ...
     * when ENABLE is clear results in UNPREDICTABLE behaviour. When the MPU is
     * enabled: 0 = MPU is disabled during HardFault and NMI handlers,
     * regardless of the value of the ENABLE bit. 1 = the MPU is enabled during
     ...
     * clear. 0 = If the MPU is enabled, disables use of the default memory
     * map. Any memory access to a location not covered by any enabled region
     * causes a fault. 1 = If the MPU is enabled, enables use of the default
     * memory map as a background region for privileged software accesses. When
     * enabled, the background region acts as if it is region number -1. Any
     ...
     * registers. The MPU supports 8 memory regions, so the permitted values of
     * this field are 0-7.
     */
     ...
     * indicate this new region. Write: 0 = MPU_RNR not changed, and the
     * processor: Updates the base address for the region specified in the
     * MPU_RNR. Ignores the value of the REGION field. 1 = The processor:
     *                 Updates the value of the MPU_RNR to the value of the
     * REGION field. Updates the base address for the region specified in the
     * REGION field. Always reads as zero.
     */
     ...
     * fetches enabled. 1 = Instruction fetches disabled. 26:24 = AP: Access
     * permission field 18 = S: Shareable bit 17 = C: Cacheable bit 16 = B:
     * Bufferable bit
     */
