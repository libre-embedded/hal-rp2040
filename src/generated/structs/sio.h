...
{
    /* Constant attributes. */
    static constexpr struct_id_t id = 20;    /*!< sio's identifier. */
...
its FIFO_ST register. */
 ...
 REMAINDER. UDIVIDEND/SDIVIDEND are aliases of the same internal register. The
 U alias starts an unsigned calculation, and the S alias starts a signed
 calculation. */
    uint32_t DIV_UDIVISOR;  /*!< (read-write) Divider unsigned divisor
 Write to the DIVISOR operand of the divider, i.e. the q in `p / q`.
 Any operand write starts a new calculation. The results appear in QUOTIENT,
 REMAINDER.  UDIVISOR/SDIVISOR are aliases of the same internal register. The U
 alias starts an  unsigned calculation, and the S alias starts a signed
 calculation. */
 ...
 */
 ...
 CSR_READY is low.  For signed calculations, QUOTIENT is negative when the signs
 of DIVIDEND and DIVISOR differ.  This register can be written to directly, for
 context save/restore purposes. This halts any  in-progress calculation and sets
 the CSR_READY and CSR_DIRTY flags.  Reading from QUOTIENT clears the CSR_DIRTY
 flag, so should read results in the order  REMAINDER, QUOTIENT if CSR_DIRTY is
 used. */
    uint32_t DIV_REMAINDER; /*!< (read-write) Divider result remainder
 The result of `DIVIDEND % DIVISOR` (modulo). Contents undefined while
 CSR_READY is low. For signed calculations, REMAINDER is negative only when
 DIVIDEND is negative. This register can be written to directly, for context
 save/restore purposes. This halts any in-progress calculation and sets the
 CSR_READY and CSR_DIRTY flags. */
    ...
    uint32_t INTERP0_ACCUM0_ADD; /*!< (read-write) Values written here are
      atomically added to ACCUM0 Reading yields lane 0's raw shift and mask
      value (BASE0 not added). */
    uint32_t INTERP0_ACCUM1_ADD; /*!< (read-write) Values written here are
      atomically added to ACCUM1 Reading yields lane 1's raw shift and mask
      value (BASE1 not added). */
    uint32_t INTERP0_BASE_1AND0; /*!< (read-write) On write, the lower 16 bits
      go to BASE0, upper bits to BASE1 simultaneously. Each half is
    ...
    std::array<uint32_t, 32>
        SPINLOCKS; /*!< (read-write) Reading from a spinlock address will:
...
wins. The value returned on success is 0x1 << lock number. */
     ...
     * value from the pins. If core 0 and core 1 both write to GPIO_OUT
     * simultaneously (or to a SET/CLR/XOR alias), the result is as though the
     * write from core 0 took place first, and the write from core 1 was then
     ...
     *                 Reading back gives the last value written, NOT the input
     * value from the pins. If core 0 and core 1 both write to GPIO_HI_OUT
     * simultaneously (or to a SET/CLR/XOR alias), the result is as though the
     * write from core 0 took place first, and the write from core 1 was then
     * applied to that intermediate result.
     */
     ...
     * core 1 -> core 0. Both are 32 bits wide and 8 words deep. Core 0 can see
     * the read side of the 1->0 FIFO (RX), and the write side of 0->1 FIFO
     * (TX). Core 1 can see the read side of the 0->1 FIFO (RX), and the write
     * side of 1->0 FIFO (TX). The SIO IRQ for each core is the logical OR of
     * the VLD, WOF and ROE fields of its FIFO_ST register.
     */
     ...
     * immediately start a new calculation, no matter if one is already in
     * progress. Writing to a result register will immediately terminate any
     * in-progress calculation and set the READY and DIRTY flags.
     */
     ...
     *
     * Changes to 1 when any register is written, and back to 0 when QUOTIENT
     * is read. Software can use this flag to make save/restore more efficient
     * (skip if not DIRTY). If the flag is used in this way, it's recommended
     * to either read QUOTIENT only, or REMAINDER and then QUOTIENT, to prevent
     ...
     * and BASE1, controlled by the 8 LSBs of lane 1 shift and mask value (a
     * fractional number between 0 and 255/256ths)
     ...
     * added (BASE2 + lane 0 shift+mask) LANE1 SIGNED flag controls whether the
     ...
     * a lower bound of BASE0 and an upper bound of BASE1.
