...
#pragma once
#ifndef RP2040_STRUCTS_RESETS_H
...
{
    /* Constant attributes. */
    static constexpr struct_id_t id = 15;   /*!< resets's identifier. */
     ...
     *
     * (read-write) Reset control. If a bit is set it means the peripheral is
     * in reset. 0 means the peripheral's reset is deasserted.
     */
     ...
     *
     * (read-write) Watchdog select. If a bit is set then the watchdog will
     ...
     *
     * (read-only) Reset done. If a bit is set then a reset done signal has
     * been returned by the peripheral. This indicates that the peripheral's
